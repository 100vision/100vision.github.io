...
Rancher Server 可以Docker形式部署在Kubernetes之外，以可以二进制部署。
...
4. 在每个节点上配置好防火墙（或是关闭）；
5. 在Rancher Server上创建集群，生产集群配置。
6. 在每个节点上执行Rancher Server上生产配置命令，注册加入；

::: tip
Rancher Server除了支持裸金属服务器上部署，也支持 AWS/Azure/Google云主机上创建集群；
:::
