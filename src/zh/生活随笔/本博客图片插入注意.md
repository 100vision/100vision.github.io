...
![png不显示](../PostImages/post26_wrong_static_resource_file.PNG)
...
- 根据报错，Google半天也没有找到有用信息。
- 以为可能是文件目录不支持中文字符或下划线，该英文等待还不行；
- 以为是文件扩展名大写不支持，换了还不行；
- 最后试出，是编译组件vite不支持`.JPG`扩展名文件。也不知道vite是个啥，最后把JPG换成`.png`,最后居然编译通过，图片也能发布成功。