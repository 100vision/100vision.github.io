...
Recipient Policies
Message Tracking
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><ul><li>定义角色组</li></ul><p>可以自定义角色组。步骤文章后面【授权】章节介绍。</p><h3 id="角色-role" tabindex="-1"><a class="header-anchor" href="#角色-role" aria-hidden="true">#</a> 角色 （Role)</h3><ul><li>默认Role</li></ul><p>默认有很多，分得比较细，不同role执行不同管理任务：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>PS C:\\&gt; get-managementrole | select name| sort -Property name
...
WorkloadManagement
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>其中，比较用的多的角色是：</p><ul><li><p><code>Mail Recipients</code> 邮箱管理</p></li><li><p><code>Distribution Group</code> 邮件通讯组管理</p></li><li><p><code>User Options</code> 高级邮箱选项管理</p></li><li><p>定义role</p></li></ul><p>其实Role是一组Role Entry的合集，我们可以自定义role,步骤文章后面【授权】章节介绍。</p><ul><li>查看指定Role都有哪些Role Entry (即有权做什么)。例如查看角色<code>Mail Recipients</code>对应的entry:</li></ul><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>PS C:\\&gt; Get-ManagementRole  &quot;mail recipients&quot; | select -expand RoleEntries |select-string &quot;new-mailbox&quot;
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="role-entry" tabindex="-1"><a class="header-anchor" href="#role-entry" aria-hidden="true">#</a> Role Entry</h3><p><code>Role Entry</code> 是role的子集，由<code>Exchange cmdlet</code>和<code>Exchange cmdParameter</code>组成, 例如<code>Exchange cmdlet</code>常用的,新建邮箱<code>New-mailbox</code>、设置邮箱<code>Set-mailbox</code></p><div class="hint-container note"><p class="hint-container-title">注</p><p>由上可以看出Exchange RBAC就是通过控制使用cmdlet来管理权限，哪些角色可以执行哪些cmdlet。</p></div><div class="hint-container tip"><p class="hint-container-title">提示</p><p>另外，可以通过cmdlet来倒查哪些角色拥有的权限。例如，想查询哪些角色有新建邮箱的权限。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>PS C:\\&gt; Get-ManagementRole  -cmdlet &quot;new-mailbox&quot;
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div></div><h2 id="二、授权" tabindex="-1"><a class="header-anchor" href="#二、授权" aria-hidden="true">#</a> 二、授权</h2><ul><li>方法1：使用默认角色组和自定义角色组授权。</li><li>方法2：使用自定义角色授权；</li></ul><p>以上管理授权颗粒由粗到细。</p><h3 id="使用角色组" tabindex="-1"><a class="header-anchor" href="#使用角色组" aria-hidden="true">#</a> 使用角色组</h3><p>一般情况下，使用角色组授权就可以满足基于RBAC的管理需求。常用的是：</p><ul><li><code>Organization Management</code> 成员是Exchange管理员，管理组织所有设置/策略；</li><li><code>Recipient Management</code>的成员是桌面运维支持人员，可以创建邮箱和设置邮箱选项等；</li></ul><p>如果不能满足，则自定义角色组。</p><p><strong>需求场景</strong></p><p>例如，如果新建一个自定义管理角色组只允许创建邮箱，不允许管理通讯组。则可以新建一个角色组，可以考虑只给予<code>Mail Recipient Creation</code>角色，不给予<code>Distribution Groups</code>角色。</p><p><strong>开始</strong></p><p>1、登录EAC,点击【新建】或是通过【复制】，然后编辑复制出然后编辑角色； 2、 添加给新建的角色Mail Recipient Creation；不添加其他角色； 3、 如果是复制的角色组，则移除Mail Recipient Creation外的所有其他权限； 3、 添加成员；</p><h3 id="使用自定义角色" tabindex="-1"><a class="header-anchor" href="#使用自定义角色" aria-hidden="true">#</a> 使用自定义角色</h3><div class="hint-container note"><p class="hint-container-title">注</p><p>必须通过Exchange Management Shell，Exchange Admin Console不支持。</p></div><p>举例说明：</p><p><strong>需求</strong></p><p>需要一个邮箱管理员角色，改角色可以新建邮箱但不可以设置用户邮箱的【邮件转发】、【邮箱委托】。如下图：</p><figure><img src="`+d+'" alt="邮件转发" tabindex="0" loading="lazy"><figcaption>邮件转发</figcaption></figure><figure><img src="'+r+`" alt="邮件转发" tabindex="0" loading="lazy"><figcaption>邮件转发</figcaption></figure><p><strong>需求分析</strong></p><p>需要了解到<code>邮件转发</code>、<code>邮箱委托</code>对应的cmdlet和cmdParameter. 了解到：</p><ul><li><p>邮件转发对应的cmdlet是<code>Set-Mailbox</code> ,对应的parameters有 cmdParameters <code>ForwardingAddress</code>,<code>ForwardingSmtpAddress</code>,<code>DeliverToMailboxAndForwardPS</code></p></li><li><p>邮箱委托对应的cmdlet是<code>Add-mailboxPermission</code></p></li></ul><p><strong>开始</strong></p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>## 先新建一个自定义角色 My Mailbox Admins。这角色必须以已有角色为模板，本例中使用Mail Recipients为模板。
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>效果</strong></p><ul><li>改角色的成员登录EAC后，邮件流 - 邮件转发复选框是灰色的，不可以勾选。</li><li>邮箱委托添加成员后不可保存设置。</li></ul><p><strong>总结</strong></p><p>通过以上，可以比较精细控制Exchange管理权限。过程比较花时间的就是找出需要控制的cmdlet和parameters，只能多查询得到。</p>`,56),o=[t];function c(m,u){return i(),n("div",null,o)}const v=e(s,[["render",c],["__file","post53_msg_exchange_roles_intro.html.vue"]]);export{v as default};
